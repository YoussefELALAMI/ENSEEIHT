...
void barber_shop_par_fixed(int nbarbs, int nchairs){
  ...
  omp_lock_t *locksChair;
  locksChair = (omp_lock_t*)malloc(nchairs*sizeof(omp_lock_t));

  chair = 0;

  #pragma omp parallel firstprivate(barber, client, chair) shared(locksChair) 
  {
    for(;;){
      ...
      if(client<0) break;
      chair = barber%nchairs;
      omp_set_lock(&locksChair[chair]);
      serve_client(barber, client, chair);
      omp_unset_lock(&locksChair[chair]);
    }
  }
...
void barber_shop_par_any(int nbarbs, int nchairs){
  ...
  #pragma omp parallel firstprivate(barber, client, chair) shared(locksChair) 
  {
    for(;;){
      ...
      while(!omp_test_lock(&locksChair[chair])){
