...
function newton(f::Function, gradf::Function, hessf::Function, x0::Union{Real,Vector{<:Real}}; 
    ...
    gradient = gradf(x_sol)
    hessienne = 0
    d = zeros(length(x0)) # d = x_{k+1} - x_{k} (d c'est la direction de Newton)
    xk = zeros(length(x0))    
    ...
    #### IL FAUT MODIFIER LA BOUCLE ######
    #### POUR AVOIR UN SEUL APPEL DU #####
    #### GRADIENT ET DE LA HESSIENNE #####
    while flag == -1
        # Calcul du gradient et de la hessienne
        gradient = gradf(x_sol)
        hessienne = hessf(x_sol)
        # Calcul de la direction de Newton
        d = - hessienne \ gradient
